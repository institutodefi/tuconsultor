...
{
  const emp = { id:'E1', nombre:'Metalúrgica Norte, S.L.', cif:'B-84.867.670' };
  const d = {
    presupuestos: [
      { id:'A', cif:'B84867670', numero_oferta:'OF-A', estado:'aceptada' },
      { id:'B', cif:'B84867670', numero_oferta:'OF-B', estado:'rechazada' },
      { id:'C', cif:'B84867670', numero_oferta:'OF-C', estado:'caducada' },
      { id:'D', cif:'B84867670', numero_oferta:'OF-D', estado:'emitida' },
      { id:'E', cif:'B84867670', numero_oferta:'OF-E', estado:'emitida' },  // con contrato
    ],
    contratos: [
      { id:'CT1', numero:'CT-1', presupuesto_id:'E', cliente_cif:'B84867670', estado:'firmado' },
    ],
    clientes: [{ id:'CL1', empresa:'Metalúrgica Norte, S.L.', cif:'B84867670' }],
    proyectos: [
      { id:'P1', cliente_id:'CL1', estado:'activo', contrato_id:null },
      { id:'P2', cliente_id:'CL1', estado:'cerrado' },
      { id:'P3', cliente_id:'CL1', estado:'pausado' },
    ],
  };
  const R = carteraDe(emp, d).resumen;
  console.log(' aceptadas (A + E con contrato):', R.aceptadas, ok(R.aceptadas===2));
  console.log(' rechazadas (B rechazada, C caducada):', R.rechazadas, ok(R.rechazadas===2));
  console.log(' tasa sobre resueltas (2 de 4):', R.tasaAceptacion+'%', ok(R.tasaAceptacion===50));
  console.log(' emitidas sin resolver no cuentan:', ok(R.aceptadas+R.rechazadas===4));
  console.log('\n── Proyectos por estado ──');
  console.log(' activos  :', R.proyectosActivos, ok(R.proyectosActivos===1));
  console.log(' cerrados :', R.proyCerrados, ok(R.proyCerrados===1));
  console.log(' pausados :', R.proyPausados, ok(R.proyPausados===1));
  console.log(' pendientes (CT1 firmado sin proyecto):', R.proyPendientes, ok(R.proyPendientes===1));
  console.log(' contratosSinProyecto lista           :', R.contratosSinProyecto.map(c=>c.numero).join(','), ok(R.contratosSinProyecto[0]?.numero==='CT-1'));

  const d2 = { ...d, proyectos: [...d.proyectos, { id:'P4', cliente_id:'CL1', estado:'activo', contrato_id:'CT1' }] };
  const R2 = carteraDe(emp, d2).resumen;
  console.log(' al abrir el proyecto, deja de estar pendiente:', R2.proyPendientes, ok(R2.proyPendientes===0));
}
