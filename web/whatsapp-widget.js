...
(function () {
  ...
  // ---- CSS ----
  var css = '' +
  '.wa-fab{position:fixed;bottom:var(--flotante-1-abajo,20px);right:24px;z-index:var(--capa-flotante-1,9100);width:60px;height:60px;border-radius:50%;background:#25D366;border:none;cursor:pointer;box-shadow:0 6px 20px rgba(0,0,0,.25);display:flex;align-items:center;justify-content:center;transition:transform .2s ease,box-shadow .2s ease}' +
  ...
  '.wa-popup{position:fixed;bottom:var(--flotante-2-abajo,96px);right:24px;z-index:var(--capa-ventana,9500);width:320px;max-width:calc(100vw - 48px);background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 16px 48px rgba(6,27,69,.28);opacity:0;transform:translateY(12px) scale(.98);pointer-events:none;transition:opacity .22s ease,transform .22s ease;font-family:Manrope,system-ui,sans-serif}' +
  ...
  '.wa-head{background:#0E1730;color:#fff;padding:16px 18px;display:flex;align-items:center;gap:12px;position:relative}' +
  ...
  '.wa-bubble{background:#fff;border-radius:10px;padding:12px 14px;font-size:16px;color:#111;line-height:1.45;box-shadow:0 1px 2px rgba(0,0,0,.08);position:relative;max-width:90%}' +
  ...
  '.wa-time{font-size:11px;color:#575757;text-align:right;margin-top:4px}' +
  '.wa-w-rtl .wa-time{text-align:left}' +
  '.wa-foot{padding:14px 18px 18px;background:#fff}' +
  '.wa-input{width:100%;border:1px solid #d5d5d5;border-radius:22px;padding:11px 16px;font-size:16px;font-family:inherit;outline:none;box-sizing:border-box}' +
  '.wa-input:focus{border-color:#0A2A6C}' +
  '.wa-send{margin-top:10px;width:100%;border:none;border-radius:22px;cursor:pointer;background:#25D366;color:#06281A;font-weight:800;font-size:15px;font-family:inherit;padding:12px;display:flex;align-items:center;justify-content:center;gap:8px;transition:background .18s ease}' +
  '.wa-send:hover{background:#1eb855}' +
  '.wa-send svg{width:18px;height:18px;fill:#fff}' +
  '@media (max-width:480px){.wa-fab{bottom:16px;right:16px}.wa-w-rtl .wa-fab{left:16px;right:auto}.wa-popup{bottom:calc(var(--flotante-2-abajo,96px) + 40px);right:16px}.wa-w-rtl .wa-popup{left:16px;right:auto}}';
  ...
  function build() {
    ...
    if (isRTL) wrap.className = 'flotante-apilado wa-w-rtl';
    wrap.innerHTML =
    ...
    function openPopup() {
      popup.classList.add('open'); document.body.classList.add('wa-abierto');
      if (dot) dot.style.display = 'none';
      setTimeout(function () { try { input.focus(); } catch (e) {} }, 250);
      push('whatsapp_open', {});
    }
    function closePopup() { popup.classList.remove('open'); document.body.classList.remove('wa-abierto'); }
