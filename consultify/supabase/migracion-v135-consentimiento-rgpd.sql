...
-- acepta queda en `consentimientos` con fecha, canal, versión del texto, IP y
...
create index if not exists consentimientos_contacto_idx on public.consentimientos (contacto_id, fecha desc);

alter table public.consentimientos enable row level security;
drop policy if exists cons_equipo_lee on public.consentimientos;
create policy cons_equipo_lee on public.consentimientos for select to authenticated using (public.es_equipo());
drop policy if exists cons_equipo_escribe on public.consentimientos;
create policy cons_equipo_escribe on public.consentimientos for insert to authenticated with check (public.es_equipo());
-- La aceptación por enlace la escribe la función de Netlify con la clave de servicio.

comment on table public.consentimientos is 'Prueba del consentimiento RGPD de cada contacto: fecha, canal, versión del texto, IP y navegador.';
